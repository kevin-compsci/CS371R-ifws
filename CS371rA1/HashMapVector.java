 ...
 *
 * @author Ray Mooney
 * Additional features by Kevin Nguyen (kdn433) for CS371r project 1 in Fall 2016.
 */
...
public class HashMapVector {
  ...
  /* sortThisMap function will sort a mapping by values
  * Input: Map<String, Integer> map
  * Output: Map<String, Integer> map (sorted)
  */
  public Map<String, Integer> sortThisMap(Map<String, Integer> map, int maxPhrase) {  
    /* Local Declarations */
    int phraseCount = 0;
    Set<Map.Entry<String, Integer>> entrySets = map.entrySet();
    List<Map.Entry<String, Integer>> tempMapList = new ArrayList<Map.Entry<String, Integer>>(entrySets);
    LinkedHashMap<String, Integer> sortedMap = new LinkedHashMap<String, Integer>(tempMapList.size());

    /* Comparator to help sort unordered hashmap */
    Comparator<Map.Entry<String, Integer>> compareValues = new Comparator<Map.Entry<String, Integer>>() { 
      @Override /* compare integer values for descending ordering */
      public int compare(Map.Entry<String, Integer> valueA, Map.Entry<String, Integer> valueB) { 
        Integer tempA = valueA.getValue(), tempB = valueB.getValue(); 
        return tempB.compareTo(tempA); 
      } 
    };

    /* sort the tempMapList (list version of the hashmap) */
    Collections.sort(tempMapList, compareValues);
      
    /* iterate through the tempMapList that was just sorted and put them into a linkedHashMap
    * to maintain ordering. Only top 1000 will be placed into the returned map
    */
    for(Map.Entry<String, Integer> item : tempMapList) {
      if (phraseCount < maxPhrase) {
        System.out.println("\nPhrase: " + item.getKey() + " " + item.getValue());
        phraseCount++;
      }
      else {
  ...
  /* removeLowRanked function will take the vector and remove the lowest bigram phrase
  * Input: Map<String, Integer> map and Map<String, TokenInfo> tokenHash
  * Output: N/A
  */
  public void removeLowRanked(Map<String, Integer> map, Map<String, TokenInfo> tokenHash) {
    /* Local Declarations */
    String token = "";

    /* iterate through vector to find removable item */
    Iterator<Map.Entry<String, TokenInfo>> iterate = tokenHash.entrySet().iterator(); 
    while(iterate.hasNext()) {
      Map.Entry<String, TokenInfo> entry = iterate.next();
      token = entry.getKey();
      /* if original map doesn't have key then remove it from the index list */
