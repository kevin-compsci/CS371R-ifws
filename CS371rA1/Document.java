...
public abstract class Document {
  ...
  /* determineQuery function will decide whether the query is only a single token entry or multiple tokens
  * Input: none
  * Output: HashMapVector vector
  */
  public HashMapVector determineQuery() {
    /* Local Declarations */
    String phrase = "", token = "";
    if (numTokens != 0) {
      return null;
    ...
    /* check if there is a next token at all, if not then it is a single token otherwise multiple tokens */
    if (nextToken == null) {
      getQueryToken(token, vector);
    }
    else {
      getQueryPhrase(phrase, token, vector);
    }
    /* return the query vector */
    return vector;
  }

  /* getQueryToken will process the token that is a single query token
  * Input: String token, HashMapVector vector
  * Output: none
  */
  public void getQueryToken(String token, HashMapVector vector) {
      // Process each token in the document and add it to the vector
      while (hasMoreTokens()) {
        token = nextToken();
        vector.increment(token);
  ...
  /* getQueryPhrase function will concatenate tokens to get a full phrase to index with
  * Input: String phrase, String token, HashMapVector vector
  * Output: none
  */
  public void getQueryPhrase(String phrase, String token, HashMapVector vector) {
    // Process each token in the document and add it to the vector
    while (hasMoreTokens()) {
      token = nextToken();
      phrase += token;
    }
    vector.increment(phrase);
  }
