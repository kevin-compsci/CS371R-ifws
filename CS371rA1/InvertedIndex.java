...
public class InvertedIndex extends InvertedPhraseIndex {
  ...
  /* constructor for the super class InvertedPhraseIndex */
  ...
  protected void indexDocuments() {
    ...
    /* filter out extra bigrams and keep only top 1000 high frequency bigrams */
    super.filterBigrams(tokenHash);
  ...
  protected void indexDocument(FileDocument doc, HashMapVector vector) {
    ...
    for (Map.Entry<String, Weight> entry : vector.entrySet()) {
      ...
      /* Initialize bigram operations here */
      initBigrams(bigramCheck, token, vector, count, docRef);
      bigramCheck = true;
    }
  }

  /* initBigrams function will initialize the scanning and consideration of potential bigrams
  * Input: String token, HashMapVector vector, int count, and DocumentReference docRef
  * Output: none
  */
  public void initBigrams(boolean bigramCheck, String token, HashMapVector vector, int count, DocumentReference docRef) {
      /* check for bigrams */
      super.bigramProcess(token, vector, bigramCheck, count);

      /* add phrase (bigrams) into index */
      if (bigramCheck) {
        indexToken(super.phrase, super.freq, docRef);
      }
  }
