 ...
 *
 * @author Ray Mooney
 * Additional features by Kevin Nguyen (kdn433) for CS371r project 2 in Fall 2016.
 */
public class InvertedIndex extends InvertedPhraseIndex {
  /* Global Variables */
  public static int maxDocs = 0;  //max number of relevant documents for pseudofeedback
  public static Double alpha = 1.0, beta = 1.0, gamma = 1.0;  //values for ALPHA, BETA, and GAMMA feedback parameters
  ...
  /**
  * Whether relevance pseudofeedback using the Ide_regular algorithm is used
  */
  ...
  */
   ...
   */
  public InvertedIndex(File dirFile, short docType, boolean stem, boolean feedback, boolean pseudofeedback, boolean feedbackparams, int maxDocs, double alpha, double beta, double gamma) {
  ...
  /*
  * initiatePseduoFeedback function will begin the pseudofeedback operations by getting top documents
  * as relevant and returning the new query
  * INPUT: Feedback fdback
  * OUTPUT: NONE
  */
  public Retrieval[] initiatePseudoFeedback(Feedback fdback, Retrieval[] retrievals) {
    pseudofeedback = false;
    /* Call the feedback function getPseudoFeedback, Feedback class should handle all feedback type operations */
    fdback.getPseudoFeedback();
    /* get new retrievals from the revised query */
  ...
  public Retrieval[] retrieve(HashMapVector vector) {
    ...
    /* immediately execute pseudofeedback where we assume the top documents are relevant (good docs) */
    if (pseudofeedback && !feedback) {
      /* constructor for pseudofeedback */
      Feedback fdback = new Feedback(vector, retrievals, this, maxDocs, alpha, beta, gamma);  //maxDocs added for pseudofeedback, maxDocs = top documents for revised query
      /* Initiate pseudofeedback and obtain the revised and retrieved documents */
      retrievals = initiatePseudoFeedback(fdback, retrievals);
      pseudofeedback = true;
    }
    
    /* return retrievals after applying pseudofeedback */
    return retrievals;
  }
  ...
  public void processQueries() {
    ...
    do {
      ...
      HashMapVector queryVector = (new TextStringDocument(query, stem)).determineQuery();
  ...
  public void presentRetrievals(HashMapVector queryVector, Retrieval[] retrievals) {
    if (showRetrievals(retrievals)) {
      ...
      if (feedback)
        fdback = new Feedback(queryVector, retrievals, this, maxDocs, 1.0, 1.0, 1.0);  //alpha, beta, gamma, and maxDocs added for pseudofeedback, maxDocs = top documents for revised query
  ...
  /*
  * getFeedbackParamValues void function will get and set the values for --> alpha, beta, and gamma
  * paramValueTracker will allow which variable to be set first. 0 for alpha, 1 for beta, and 2 for gamma.
  * OUTPUT: None
  */
  public static void getFeedbackParamValues(String flag, int paramValueTracker) {
    /* Get values for individual floating point arguments for Alpha, Beta, and Gamma values */
    if (paramValueTracker == 0) {
      //Parse from string to double to get the first numeric value argument for alpha
      alpha = Double.parseDouble(flag);  
    }
    else if (paramValueTracker == 1) {
      //Parse from string to double to get the second numeric value argument for beta
      beta = Double.parseDouble(flag);
    }
    else {
      //Parse from string to double to et the third (last) numeric value argument for gamma
      gamma = Double.parseDouble(flag);
    }
  }

  /*
  * checkForNumeric boolean function will take a string input and check if each letter is a valid digit 
  * else return false
  * INPUT: String line
  * OUTPUT: true or false
  */
  public static boolean checkForNumeric(String line) {
    /* Local variables */
    int counter = 0;
    char letter;

    /* loop to iterate through each letter in the string input */
    while (counter < line.length()) {
      //get letter
      letter = line.charAt(counter);

      //Exception is a dot for decimal representation, continue and increment counter as normal
      if (letter == '.') {
        counter++;  //increment to get next letter
        continue;  //skip everything below since the dot is valid
      }

      //check by ASCII if current letter is not a digit
      if ((letter < 48) || (letter > 57)) {
        /* return false if atleast one letter is not a digit */
        return false;
      }
      counter++;
    }
    /* return true since every letter is a digit */
    return true;
  }
  ...
  public static void main(String[] args) {
    ...
    int paramValueTracker = 0;  //0 to set alpha value, 1 to set beta value, 2 to set gamma value
    ...
    for (int i = 0; i < args.length - 1; i++) {
      ...
      if (flag.equals("-html"))
        ...
        // Use pseudo feedback and set condition to true
        pseudofeedback = true;
      }
      else if (checkForNumeric(flag) && pseudofeedback && !feedbackparams) {  //is flag an actual number? and pseudofeedback is true
        // Get numeric value if any from flag
        maxDocs = Integer.parseInt(flag);
      }
      else if (flag.equals("-feedbackparams")) {
        // if feedback parameters are provided in args
        feedbackparams = true;
      }
      else if (checkForNumeric(flag) && feedbackparams) { //is flag an actual value and feedbackparams is true?
        /* Get values for alpha, beta, and gamma */
        getFeedbackParamValues(flag, paramValueTracker);
        paramValueTracker++;
      }
      else {
    ...
    InvertedIndex index = new InvertedIndex(new File(dirName), docType, stem, feedback, pseudofeedback, feedbackparams, maxDocs, alpha, beta, gamma);
    // index.print();
