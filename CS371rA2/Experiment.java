...
public class Experiment {
  ...
  */
   ...
   */
  ArrayList<ArrayList<RecallPrecisionPair>> rpResults = new ArrayList<ArrayList<RecallPrecisionPair>>();
  ...
  public Experiment(File corpusDir, File queryFile, File outFile, short docType, boolean stem)
    ...
    this.index = new InvertedIndex(corpusDir, docType, stem, false, pseudofeedback, feedbackparams, 0, 1.0, 1.0, 1.0);
   ...
   * @param stem      Whether tokens should be stemmed with Porter stemmer.
   */
  public Experiment(File corpusDir, File queryFile, File outFile, short docType, boolean stem, boolean pseudofeedback, boolean feedbackparams, int maxDocs)
      throws IOException {
    this.pseudofeedback = pseudofeedback;
    this.feedbackparams = feedbackparams;
    this.maxDocs = maxDocs;
    this.corpusDir = corpusDir;
    this.index = new InvertedIndex(corpusDir, docType, stem, false, pseudofeedback, feedbackparams, maxDocs, alpha, beta, gamma);
    this.queryFile = queryFile;
    this.outFile = outFile;
  }
   ...
   */
  ArrayList<RecallPrecisionPair> evalRetrievals(Retrieval[] retrievals, ArrayList<String> correctRetrievals) {
    ...
    for (int i = 0; i < retrievals.length; i++) {
      ...
      if (correctRetrievals.contains(retrievals[i].docRef.file.getName())) {
        ...
        System.out.println(MoreString.padToLeft(numRetrieved, 4) + " is relevant; Recall = " +
            MoreString.padToLeft(MoreMath.roundTo(100 * recall, 3) + "%", 7) + "; Precision = " +
  ...
  */
  public static boolean checkForNumeric(String line) {
    ...
    while (counter < line.length()) {
      //get letter from numeric string
      ...
      if (letter == '.') {
        ...
        continue;  //skip everything below in the loop since the dot is valid
      }
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < args.length - 3; i++) {
      ...
      else if (checkForNumeric(flag) && pseudofeedback && !feedbackparams) {
        // Get feedback value if any
        maxDocs = Integer.parseInt(flag);
      }
      else if (flag.equals("-feedbackparams")) {
        // if feedback parameters are provided
        feedbackparams = true;
      }
      else if (checkForNumeric(flag) && feedbackparams) {
        /* set values for alpha, beta, and gamma based on paramValueTracker */
    ...
    Experiment exper = new Experiment(new File(corpusDir), new File(queryFile), new File(outFile), docType, stem, pseudofeedback, feedbackparams, maxDocs);
    exper.makeRpCurve();
  }
}